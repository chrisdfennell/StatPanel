-- Locales/esMX.lua (Spanish, Latin America)
--
-- Derived from esES.lua, which the Latin American client tracks closely: the
-- two Blizzard localizations differ in scattered vocabulary rather than in
-- structure. Keeping this file a derivation rather than a hand-copy means the
-- key set cannot silently drift out of step with esES.
--
-- Where a term genuinely differs in the esMX client, override it below the
-- shared block and note why. Run the linter after editing.
--
-- NOT REVIEWED BY A NATIVE SPEAKER. Corrections are very welcome -- please open
-- an issue at https://github.com/chrisdfennell/StatPanel/issues.

local _, SP = ...
local L = SP.Locale("esMX")
...
L["Updated for World of Warcraft patch 12.1.0."] = "Actualizado para el parche 12.1.0 de World of Warcraft."
