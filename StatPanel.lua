 ...
 CreateStatPanel = function()
     ...
     local maxPlayerLevel = GetMaxPlayerLevel() -- Fetch the current max player level dynamically -- Dynamically fetch the max player level for the current expansion
         -- Get the player's item level
         local avgItemLevel, avgItemLevelEquipped = GetAverageItemLevel()
         statFrame.title:SetText(string.format("iLvl: %d", math.floor(avgItemLevelEquipped + 0.5)))
         ...
         if statText["Crit"] then
             statText["Crit"]:SetText(string.format("Crit: %d%%", math.floor(crit + 0.5)))
             statBars["Crit"]:SetMinMaxValues(0, 100)
             statBars["Crit"]:SetValue(crit)
         end
         if statText["Haste"] then
             statText["Haste"]:SetText(string.format("Haste: %d%%", math.floor(haste + 0.5)))
             statBars["Haste"]:SetMinMaxValues(0, 100)
             statBars["Haste"]:SetValue(haste)
         end
         if statText["Mastery"] then
             statText["Mastery"]:SetText(string.format("Mastery: %d%%", math.floor(mastery + 0.5)))
             statBars["Mastery"]:SetMinMaxValues(0, 100)
             statBars["Mastery"]:SetValue(mastery)
         end
         if statText["Versatility"] then
             statText["Versatility"]:SetText(string.format("Versatility: %d%%", math.floor(versatility + 0.5)))
             statBars["Versatility"]:SetMinMaxValues(0, 100)
             statBars["Versatility"]:SetValue(versatility)
         end
         if statText["Armor"] then
             if UnitExists("target") then
                 statText["Armor"]:SetText(string.format("Damage Reduction: %d%%", math.floor(damageReductionCurrentTarget + 0.5)))
                 statBars["Armor"]:SetValue(damageReductionCurrentTarget)
             else
                 statText["Armor"]:SetText(string.format("Damage Reduction: %d%%", math.floor(damageReductionEvenlyMatched + 0.5)))
                 statBars["Armor"]:SetValue(damageReductionEvenlyMatched)
             end
             statBars["Armor"]:SetMinMaxValues(0, 100)
         end
         if statText["Dodge"] then
             statText["Dodge"]:SetText(string.format("Dodge: %d%%", math.floor(dodge + 0.5)))
             statBars["Dodge"]:SetMinMaxValues(0, 100)
             statBars["Dodge"]:SetValue(dodge)
         end
         if statText["Leech"] then
             statText["Leech"]:SetText(string.format("Leech: %d%%", math.floor(leech + 0.5)))
             statBars["Leech"]:SetMinMaxValues(0, 100)
             statBars["Leech"]:SetValue(leech)
         end
         if statText["Avoidance"] then
             statText["Avoidance"]:SetText(string.format("Avoidance: %d%%", math.floor(avoidance + 0.5)))
         ...
         if statText["Speed"] then
             ...
             statText["Speed"]:SetText(string.format("Speed: %d%%", math.floor(speed + 0.5)))
         ...
         if SPAddonDB.showFPS then
             ...
             fpsText:SetText(string.format("FPS: %d", math.floor(fps + 0.5)))
             fpsText:Show()
         else
             fpsText:Hide()
         end
     ...
     statFrame:SetScript("OnEvent", function(self, event)
         if event == "PLAYER_TARGET_CHANGED" or event == "PLAYER_EQUIPMENT_CHANGED" or event == "PLAYER_ENTERING_WORLD" then
             UpdateStats()
         end
     end)
